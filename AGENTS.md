...
policy, default submission manifest path, allowed submission file types, first-run size limits, and checker output
limits. Each present Lake package has its own package-local `lean-toolchain` file recorded by manifest; checkers
...
`manifest.config.yaml` is the source of truth for submission manifest shape. Keep
...
The submission-structure model uses statement package terminology, structured proof manifest, and statement-level proof certificates. Keep documentation aligned with `manifest.config.yaml`, `import-submission-expectations.md`, and the executable tests, in that order.
...
fixture execution, and final proof composition. The manifest file is the source of submission information. Run checks with:

```sh
node .github/actions/test/run-all.mjs --manifest=path/to/manifest.yaml
```

Pass only one manifest file path, preferably with `--manifest=path/to/manifest.yaml`. The older positional form is accepted for compatibility. If no manifest path is provided, each check falls back to `manifest.yaml` in the current working directory; directories are not accepted.
...
`general/manifest-check.mjs` validates manifest structure against `manifest.config.yaml` with Ajv before running
non-file semantic checks that the schema cannot express, such as proof entries targeting manifest `Axiom`
statements. File-existence checks belong in `general/files-present.mjs`.

The CLI test command uses the same manifest convention:

```sh
lml test --manifest=path/to/manifest.yaml
```

If no manifest path is provided, `lml test` checks `manifest.yaml` in the current working directory and returns an error when that file is missing.
...
Each submission consists of statement entries and proofs, which are decoupled: statements publish a submission's public `Definition` and `Axiom` declarations, while a proof entry discharges any statement axiom — its own submission's or another submission's. Statement files should introduce axioms rather than theorem declarations for axiom entries. Each proof manifest entry has exactly two fields: `axiom`, the global name of the statement axiom it discharges, and `proof`, the global name of the proof declaration. Both are global Lean names whose leading namespace segment is the owning submission's slug in PascalCase, so they are unique across submissions; the checker resolves both by name in the built packages and asks Lean `isDefEq` to compare their compiled types. Proof files may rely on definitions, declared dependencies, Std, and Mathlib. The proof checker builds the proof package, rejects build output that reports `sorry` or `sorryAx`, and asks Lean for each manifest proof theorem's compiled axiom dependencies so submitted proofs may not depend on `sorryAx` or local proof-namespace axioms.

The import workflow's final proof build copies the manifest-root package into isolation, runs a clean Lake build, rejects `sorry` output, composes each submitted proof target onto the statement axiom it discharges, and accepts only allowed base axioms. The remaining target work is to import all nested imported submissions into the root Lake file, recursively follow manifest references during the Lean build, rewrite references to proved dependency statements to their proof counterparts while leaving conjectures as conjectures, use the resulting `.olean` files for axiom testing, and compare computed dependency/conjecture information to manifest.
...
The `.github/workflows/submit.yml` workflow creates or updates a GitHub issue for a submission manifest file. It reads `submissionIssueNumber` from the manifest file when present, otherwise creates a new issue, labels it `submission`, writes `submissionIssueNumber` and `submissionIssueUrl` back to the manifest file, and commits that manifest update to the source branch. The issue title is the submission title, and the issue body starts with the submission abstract before recording the submitting account login, current repository URL, source branch, source commit, manifest file path, and generated GitHub source URLs. Metadata itself records local paths such as `abstractPath`, not source URLs; URLs are generated from the repository and path.

## Submission status command

The CLI `submission-status` command reports whether a manifest file has a submission issue, whether it has been imported, the related upload/test/finalizing workflow state when available, the distance from the recorded source commit to the current commit, and whether manifest statement files were added, changed, or removed since that source commit.
...
The CLI `agent-submission-guide` command prints `agent-info/paper-submission-readiness-agent-guide.md`, the guide for agents preparing an arbitrary Lean project as a Lean Meta Library paper submission with user-approved manifest, statement files, and proofs that should pass the local checks.

## Submission structure rework

`agent-info/submission-api-structure-agent-readme.md` records the current submission structure and remaining target rework. Statement-package and proof-package presence is optional, but the package roles remain separate when both are present; do not mix statement and proof content in one package. Package checks run only when the corresponding package is present, statement entries are `Definition` and `Axiom`, manifest uses the exact names in `manifest.config.yaml`, proof-level declaration reference manifest is supported, and final proof checking is built around statement-level proof certificates.

## Import submission workflow

The `.github/workflows/import-submission.yml` workflow runs when an issue labeled `submission` is opened, labeled, edited, or reopened. It reads the repository URL, source branch, source commit, manifest file path, and submitted-by login from the issue body, checks out that exact commit, runs the first-run checks from `.github/actions/test/` against the manifest file at that path, then adds or updates the matching submission row in `submissions.jsonl`. Imported rows include the parsed manifest plus source repository, branch, commit, manifest path, source-repository-relative package folder information, issue id/number/url, and submitting user id/login. After a successful import, the workflow comments on and closes the issue.
...
- Metadata should use `manifestVersion`, `statementRoot`, `proofRoot`, `submissionSlug`, `submissionTitle`, `abstractPath`, `licensePath`, `bibtex-entries`, `statements`, `proofs`, `Statement`, `DeclarationReferences`, `LakeStatementPackage`, and `LakeProofPackage`. `licensePath` must point to a file whose content contains a recognized license identifier from `lml-env.json`'s `submission.allowedLicenseIdentifiers`. Each proof entry has only `axiom` and `proof` global names. `statementRoot` and `proofRoot` are folder paths; each must contain a `lakefile.lean` and a `lean-toolchain` file.
...
- Each proof entry names the discharged statement axiom through `axiom` and the proof declaration through `proof`, both as global Lean names; the proof declaration must build in the proof package.
