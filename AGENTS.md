...
- Metadata should use `manifestVersion`, `statementRoot`, `proofRoot`, `submissionSlug`, `submissionTitle`, `bibtex-entries`, `statements`, `proofs`, `Statement`, `DeclarationReferences`, `LakeStatementPackage`, and `LakeProofPackage`. Each proof entry has only `axiom` and `proof` global names. `statementRoot` and `proofRoot` are folder paths; each must contain a `lakefile.lean` and a `lean-toolchain` file.
