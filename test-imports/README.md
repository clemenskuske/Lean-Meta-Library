...
- `missing-proof-file-package`: metadata references a theorem proof file that is
...
- `mismatched-proof-type-package`: the proof theorem has the expected name but a
  different Lean type from the surface theorem, so the axiom-to-proof type check
  should reject it.
- `sorry-proof-package`: the proof theorem uses `sorry`, so proof axiom
  inspection should reject the `sorryAx` dependency.
- `extra-surface-declaration-package`: a theorem surface file introduces an
